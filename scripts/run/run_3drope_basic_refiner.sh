...
docker run --rm --privileged --network=host \
  ...
  -e USE_TRT_TEXT_ENCODER="${USE_TRT_TEXT_ENCODER:-0}" \
  -e TEXT_ENCODER_ENGINE_DIR="${TEXT_ENCODER_ENGINE_DIR:-$ENGINE_DIR}" \
  -e TEXT_ENCODER_GROUPS="${TEXT_ENCODER_GROUPS:-}" \
  -e DEBUG_TENSOR_STATS="${DEBUG_TENSOR_STATS:-0}" \
  -e MINIMAL_PYTORCH_LOAD="${MINIMAL_PYTORCH_LOAD:-1}" \
  -e USE_TRANSFORMERS_TOKENIZER="${USE_TRANSFORMERS_TOKENIZER:-0}" \
  -e USE_DIFFUSERS_SCHEDULER="${USE_DIFFUSERS_SCHEDULER:-0}" \
